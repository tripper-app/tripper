...
export class SignUpComponent implements OnInit {
    ...
    loginWithFacebook() {
        facebook.login((err, fbData) => {
            if (err) {
                ...
                console.log("GOOD");
                this.loginWithThirdParty(fbData.token, 'facebook')
    ...
    loginWithThirdParty(token, thirdParty) {
        ...
        this.httpService.loginWithThirdPartyToken(token, thirdParty).subscribe((res: { email: string }) => {
            console.log(res);
            this.saveTokenToCache(res.email);
        }, err => this.handleError(err))
    }
