...
export const login = functionBuilder(async (req, res) => {
    try {
...
export const loginWithThirdParty = functionBuilder(async (req, res) => {
    try {
        const token = req.query.token as string;
        const thirdrdParty = thirdParties.find(t => t.name === req.query.thirdrdParty as string);
        if (thirdrdParty) {
...
export const resetPasswordCreateCode = functionBuilder(async (req, res) => {
    try {
...
export const resetPasswordRecieveCode = functionBuilder(async (req, res) => {
    try {
...
export const changePassword = functionBuilder(async (req, res) => { // should user provide old password?
    try {
        ...
        const usersRef = await db.collection("users")
            .where("email", "==", email)
            .where("password", "==", getHash(req.body.oldPassword as string))
            .limit(1);
        const user = await usersRef.get();
        if (newPassword) {
            if (user.empty) {
...
export const getFavoriteSprings = functionBuilder(async (req, res) => {
    try {
        ...
        let springsData: any = [];
        const actualSprings: any = [];
        if (favorites) {
            favorites.forEach(async (element: any) => {
...
export const getHistorySprings = functionBuilder(async (req, res) => {
    try {
        ...
        const actualSprings: any = [];
        if (history) {
            history.forEach(async (element: any) => {
                springPromises.push(db.collection('springs').doc(element).get());
            });
        }
        springsData = await Promise.all(springPromises);
...
export const removeUser = functionBuilder(async (req, res) => {
    try {
        ...
        const commentUpdates: Promise<unknown>[] = [];
        springsSnap.forEach(doc => {
            const comments = doc.get('comments');
            if (Array.isArray(comments) && comments.some((c: any) => c.email === email)) {
                const remaining = comments.filter((c: any) => c.email !== email);
        ...
        try {
            await firebase.storage().ref(`users/${email}/profile`).delete();
        } catch (storageErr) {
...
const updateField = (field: any, language: string) => {
